(function () {
  ...
  if (products.length) {
    cart[0]
      ...
      .getElementsByTagName('span')[0].innerText = products.reduce(
      (acc, product) => {
  ...
  if (cart.length > 0) {
    var cartAddBtns = document.getElementsByClassName(
        'js-cd-add-to-cart                                                                                               '
      ),
    ...
    if (products.length) {
      ...
      cartCountItems[0].innerText = products.length;
      cart[0].classList.remove('cd-cart--empty');
    }
    ...
    function initCartEvents() {
      ...
      cart[0].addEventListener('click', function (event) {
        if (event.target == cart[0]) {
          ...
          event.preventDefault();
          console.log(event.target.closest('.cd-cart__product').id);
          for (let i = 0; i < products.length; i++) {
            if (
              products[i].id === event.target.closest('.cd-cart__product').id
            ) {
              products.splice(i, 1);
              break;
            }
    ...
    function addProduct(price, name, url, id) {
      productId = id;
      var productAdded =
        '<li class="cd-cart__product" id=' +
        productId +
        '><div class="cd-cart__image"><img src=' +
        url +
        '><a href="#0"></a></div><div class="cd-cart__details"><h3 class="truncate"><a href="#0">' +
        name +
